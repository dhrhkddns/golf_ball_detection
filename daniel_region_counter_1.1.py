...
def run(
    ...
    while videocapture.isOpened():
        ...
        if results[0].boxes.id is not None:
            ...
            #라인에 두면 게임 시작, 코드 돌아가는지 확인해야함. 갱신하지말고 파일따로 업로드
            while current_region[3]['polygon'].contains(Point((bbox_center[0], bbox_center[1]))):
                ...
                if current_region[3]['in'] == True and  bbox_center[0] < list(current_region[3]['polygon'].exterior.coords)[0][0]:
                    print("########################################")
                    hit = True #bbox_center[0]과 [1]이 움직인다 이때부터

                    before_x = bbox_center[0] #움직였을때 x좌표 저장 (0.5초 기준으로 차이 측정)
                    before_time = time.time()

                    if time.time() - before_time > 0.5: #0.5초마다 확인 계산량이 많아질까봐 그리고 너무 늦게 확인하면 지루하니까
                        if abs(before_x - bbox_center[0]) < 2: #픽셀이 2이하로 변동일때 (이정도면 거의 멈춘거임, 사실 멈춰도 1정도는 변함 그많은 것중에 2면 멈춘거지)
                            score += 1 #스코어 1 증가, (멈춤!!) 그리고 여기서 다시 쳐야함.
                            hit = False #끝났으니까 다시 초기 false값으로

                            #그리고 여전히 그린존이고 스코어 1증가했다구 사운드 넣기
                            #임의의 소리 코드 
