...
def run(
    ...
    while videocapture.isOpened():
        ...
        if results_holes[0].boxes.id is not None and results_greenzone[0].masks is not None:
            ...
            if len(ball_prepare) > 30:
                ball_prepare.pop(0)
            
            if len(ball_prepare) == 30 and once == 0:
                
                if ball_prepare.count(1) == 30:
            ...
            if ready == True:
                ...
                if ball_prepare[-1] == 0 or once == 1:
                    ...
                    if sum(big_surrounding_pixels_hsv) >= 5000:
                        ...
                        once = 0
                        sound = 0
                    if ball_center is not None and counting_regions[2]['polygon'].contains(Point((ball_center[0], ball_center[1]))) == False:
                        golf_out_sound()
